...
class I2CPacket:
    ...
    def create_pkt(data: bytes, size: int, status: str, sequence: int, ID: str):
        """
...
class Nano_I2CBus:
    ...
    def write_pkt(self, response, status, sequence):
        ...
        """
        pkt = I2CPacket.create_pkt(
            response, len(response), status, sequence, self.pkt_self_id
        )
