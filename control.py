...
def get3Dlocation(realWorldCords):
    return (
        realWorldCords[0] ** 2 + realWorldCords[1] ** 2 + realWorldCords[2] ** 2
    ) ** 0.5
...
def translateCoordinates(x, y, depth):
    ...
    real_z = (depth**2 - groundhyp**2) ** 0.5
    # print(math.atan(y/center_depth))
    # print(groundhyp)
    real_y = (groundhyp**2 - x**2) ** 0.5
    ...
    for i in range(5):
        for j in range(5):
            if j != i:
                if (
                    abs(
                        get3Dlocation(realWorldCords[i])
                        - get3Dlocation(realWorldCords[j])
                    )
                    < threshold
                ):
                    match[j] = 1
...
def collectTubeLocation(vis):
    ...
    while good < 5 and consecutiveBad < 10 and consecutiveNone < 10:
        ...
        if data[2] == -1:
            ...
            realWorldCords.append(
                translateCoordinates(data[0], data[1], data[2]) + (data[3],)
            )
            print("Real World Coordinates (X,Y,Z):", realWorldCords[good])
    ...
    if consecutiveNone >= 10:
        ...
        return checkTubeLocationValidity(
            realWorldCords
        )  # tuple(x/5 for x in realWorldCords)
...
def main():
    ...
    while True:
        ...
        if (pkt[I2CPacket.id_index].decode() != i2c.pkt_targ_id) or (
            pkt[I2CPacket.stat_index] != b"c"
        ):
            continue
